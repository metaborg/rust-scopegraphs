...
impl UnionFind {
    ...
    pub(super) fn fresh(&mut self) -> TypeVar {
        let old = self.vars;
        self.vars += 1;
    ...
    pub(super) fn unify(&mut self, left: PartialType, right: PartialType) {
    ...
    pub(super) fn find_partial_type(&mut self, ty: PartialType) -> PartialType {
    ...
    pub(super) fn type_of_partial_type(&mut self, var: PartialType) -> Option<Type> {
    ...
    pub(super) fn wait_for_unification(
        &mut self,
        tv: TypeVar,
    ) -> impl Future<Output = PartialType> {
